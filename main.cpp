...
bool init()
{
	...
	else
	{
		...
		bouncing = Mix_LoadWAV("./asset/sound/pong_bouncing.wav");
		if (bouncing == NULL)
		{
			printf("Failed to load sound effect! SDL_mixer Error: %s\n", Mix_GetError());
		}
		...
		gWindow = SDL_CreateWindow("PongPing", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
		if (gWindow == NULL)
		{
...
bool loadMedia(string a)
{
	...
	bool success = true;
	// Load splash image
	//  gBackground = SDL_ConvertSurface(IMG_Load("bg.png"), gScreenSurface->format, 0 );
...
void showInGameTwoPlayer(int a, int b, int time)
{
	TTF_Font *font = TTF_OpenFont("./asset/fonts/fontTest.ttf", 30);
	SDL_Color color = {255, 255, 255};
...
void GameManager()
{
	...
	{
		// Load media
		if (!loadMedia("Screen/start1.png"))
		{
				...
				while (SDL_PollEvent(&e) != 0)
				{
					...
					if (SDL_MOUSEBUTTONDOWN == e.type)
					{
						if (SDL_BUTTON_LEFT == e.button.button)
						{
							...
							if ((x >= 168 - 5 && x <= 348 + 5) && (y >= 192 && y <= 232))
							{
								gBackground = loadTexture("Screen/bg.png");
								OnePlayer();
							}
							else if ((x >= 168 - 5 && x <= 348 + 5) && (y >= 500 && y <= 540))
							{
								gBackground = loadTexture("Screen/bg.png");
								TwoPlayer();
							}
...
void OnePlayer()
{
	...
	SDL_Rect team2_playerRect = {250, 25 - BAR_HEIGHT, BAR_WIDTH, BAR_HEIGHT};
	SDL_Texture *player1Sprite = IMG_LoadTexture(gRenderer, "./asset/img/player.png");
	if (player1Sprite == NULL)
	{
		cout << "Failed to load player1 image! SDL_image Error" << endl;
	}
	else
		cout << "Successfully load player1 image! " << endl;
	SDL_Texture *player2Sprite = IMG_LoadTexture(gRenderer, "./asset/img/player.png");
	if (player2Sprite == NULL)
	{
		cout << "Failed to load player1 image! SDL_image Error" << endl;
	}
	else
		cout << "Success to load player2 image! " << endl;
	// SDL_RenderCopy(gRenderer, player1Sprite, NULL, &team1_player1Rect);
	// SDL_RenderCopy(gRenderer, player2Sprite, NULL, &team1_player2Rect);
	if (SDL_RenderCopy(gRenderer, player1Sprite, NULL, &team1_player1Rect) < 0)
	{
		printf("Failed to render player1! SDL Error: %s\n", SDL_GetError());
	}
	else
		cout << "good " << endl;
	if (SDL_RenderCopy(gRenderer, player2Sprite, NULL, &team1_player2Rect) < 0)
	{
		printf("Failed to render player2! SDL Error: %s\n", SDL_GetError());
	}
	else
		cout << "good2" << endl;
	...
	while (game_running)
	{
		...
		if (powerUpActive && SDL_GetTicks() - powerUpTimer > 5000) // 5000 milliseconds = 5 seconds
		{
			powerUpActive = false;
			team1_player1Rect.w /= 2; // Return player1's rectangle to its original size
		}
		...
		if (playerActive == 1)
		{
			if (state[SDL_SCANCODE_LEFT] && team1_player1Rect.x > 25)
				team1_player1Rect.x -= 6;
			if (state[SDL_SCANCODE_RIGHT] && team1_player1Rect.x < SCREEN_WIDTH - BAR_WIDTH - 25)
				team1_player1Rect.x += 6;
			if (state[SDL_SCANCODE_UP] && team1_player1Rect.y > SCREEN_HEIGHT / 2 + BAR_HEIGHT / 2)
			{
				team1_player1Rect.y -= 6;
			}
			if (state[SDL_SCANCODE_DOWN] && team1_player1Rect.y < SCREEN_HEIGHT - BAR_HEIGHT * 2)
			{
				team1_player1Rect.y += 6;
			}
		}
		...
		{
			if (state[SDL_SCANCODE_LEFT] && team1_player2Rect.x > 25)
				team1_player2Rect.x -= 6;
			if (state[SDL_SCANCODE_RIGHT] && team1_player2Rect.x < SCREEN_WIDTH - BAR_WIDTH - 25)
				team1_player2Rect.x += 6;
		}
		...
		else if (ballRect.x >= SCREEN_WIDTH - 25 - 10 || ballRect.x <= 25)
		{
			dx = dx * -1;
			touchNum += 1;
...
void TwoPlayer()
{
	...
	SDL_Texture *player1Sprite = IMG_LoadTexture(gRenderer, "./asset/img/player.png");
	SDL_Texture *player2Sprite = IMG_LoadTexture(gRenderer, "./asset/img/player.png");
	SDL_RenderCopy(gRenderer, player1Sprite, NULL, &player1Rect);
	SDL_RenderCopy(gRenderer, player1Sprite, NULL, &player2Rect);
	SDL_Rect backgroundRect = {0, 0, SCREEN_WIDTH, SCREEN_HEIGHT};
	SDL_SetRenderDrawColor(gRenderer, 0xFF, 0x00, 0x00, 0xFF);
	...
	while (game_running)
	{
		...
		if (ballRect.x >= SCREEN_WIDTH - 60 || ballRect.x <= 25)
		{
			dx = dx * -1;
			// touchNum += 1;
			Mix_PlayChannel(-1, bouncing, 0);
